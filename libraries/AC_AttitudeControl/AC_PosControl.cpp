...
void AC_PosControl::pso_read(void)
{
    ...
    printf("Position data:z_alpha=%f,z_eps=%f,xy_alpha=%f,xy_eps=%f \n", data.z_alpha,data.z_eps,data.xy_alpha,data.xy_eps);
	_pid_accel_z._alpha = data.z_alpha;
	_pid_accel_z._eps = data.z_eps;
	_pid_vel_xy._alpha =  data.xy_alpha;
	_pid_vel_xy._eps = data.xy_eps;
...
//		pso_read();
	_pid_accel_z._alpha = 0.03;
	_pid_accel_z._eps = 0.17;
	_pid_vel_xy._alpha =  127.7;
	_pid_vel_xy._eps =31.9;   
...
void AC_PosControl::run_z_controller()
{
    ...
    if (_vibe_comp_enabled) {
		...
		printf("11111111111111111111111111111133333333333333333333333333333333\n");
    } else {
		...
		float adaptive_term = _pid_accel_z.get_adaptive_term(0.001);
//		printf("Z controller333333333333333333333333  original thr_out   %f,  adaptive term %f  \n", thr_out, adaptive_term);
	...
	if(tnow > 150000 && num_z==0)
	{
		float cost = _pid_accel_z.get_cost() / 52.43766;    // 48.541

	// _kp 0.500000   _ki 1.000000   _kd 0.000000
		printf("Position control:   cost_z %f\n",(double)cost);
...
//	printf("11111111111111111111111  target height %f, height = %f and the time is %d\n", _pos_target.z, curr_alt,tnow);
...
//	printf("22222222222222222222222222222222 target x %f,target y %f,  curr_pos x %f, curr_pos y %f\n", _pos_target.x, _pos_target.y, curr_pos.x, curr_pos.y);
...
/*	bool limit;
    ...
    // acceleration to correct for velocity error and scale PID output to compensate for optical flow measurement induced EKF noise  (add adaptive term)
...
//	printf("XY controller333333333333333333333333  original X   %f, original Y   %f,  adaptive term X %f,  adaptive term Y %f,  ekfNavVelGainScaler  %f\n", accel_target.x, accel_target.y, vel_xy_adaptive_term.x, vel_xy_adaptive_term.y, ekfNavVelGainScaler);
	accel_target.x += vel_xy_adaptive_term.x * ekfNavVelGainScaler / adaptivescaler;
	accel_target.y += vel_xy_adaptive_term.y * ekfNavVelGainScaler / adaptivescaler;
	
	//get cost and writo to PSO
	uint32_t tnow = AP_HAL::millis();
	if(tnow > 150000 && num_xy==0)
	{
		float cost = _pid_vel_xy.get_cost() / 10.608;   //  10.821216

		//  xy _kp 2.000000   _ki 1.000000   _kd 0.500000
		printf("Position control:  xy cost %f \n", cost);
...
static float uroll[10000] = {0};
static float upitch[10000] = {0};
static uint32_t i = 0;
static uint32_t freq = 0;
uint32_t k = 0;

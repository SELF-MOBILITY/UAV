...
void AC_AttitudeControl_Sub::rate_controller_run()
{
    ...
    _motors.set_roll(get_rate_roll_pid().update_all(_rate_target_ang_vel.x, gyro_latest.x, _motors.limit.roll) + get_rate_roll_pid().get_adaptive_term(1));
    _motors.set_pitch(get_rate_pitch_pid().update_all(_rate_target_ang_vel.y, gyro_latest.y, _motors.limit.pitch) + get_rate_pitch_pid().get_adaptive_term(1));
    _motors.set_yaw(get_rate_yaw_pid().update_all(_rate_target_ang_vel.z, gyro_latest.z, _motors.limit.yaw) + get_rate_yaw_pid().get_adaptive_term(1));
	
		//get cost and writo to PSO
	uint32_t tnow = AP_HAL::millis();
	if(tnow>47000)
	{
		sum_roll += fabs(_motors.get_roll());
		sum_pitch += fabs(_motors.get_pitch());
		sum_yaw +=fabs( _motors.get_yaw());
		sum_throttle += fabs(_motors.get_throttle_out());
		count++;
	}
	if(tnow > 170000 && num==0)
	{
		float cost_roll = get_rate_roll_pid().get_cost() / 1;    //  0.007923
		float cost_pitch = get_rate_pitch_pid().get_cost() / 1;   //  0.007692
		float cost_yaw = get_rate_yaw_pid().get_cost() / 1;      //  0.014
		float cost_energy = (sum_roll+sum_pitch+sum_yaw+sum_throttle) / (count* 1);
		...
		printf("Attitude control: cost_roll %f, cost_pitch %f, cost_yaw %f, cost_energy %f \n", (double)cost_roll, (double)cost_pitch, (double)cost_yaw,  cost_energy);

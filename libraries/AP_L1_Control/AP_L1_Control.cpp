...
float AP_L1_Control::loiter_radius(const float radius) const
{
    ...
    if (is_zero(sanitized_bank_limit) || is_zero(nominal_velocity_sea_level) ||
        ...
        // with the current altitude, but will at least protect the airfram
//		printf("22222222222221 %f\n",eas2tas_sq);
...
void AP_L1_Control::update_loiter(const struct Location &center_WP, float radius1, int8_t loiter_direction)
{
    struct Location _current_loc;

    // scale loiter radius with square of EAS2TAS to allow us to stay
    // stable at high altitude
    float radius = loiter_radius(fabsf(radius1));
	//printf("1111111111111111111111111111111111111radius1 %f  radius %f\n",radius1,radius);
...
//printf("2221111111111111111111111111111111111 A_air.length %f   xtrackErrCirc %f\n", A_air.length(), xtrackErrCirc);

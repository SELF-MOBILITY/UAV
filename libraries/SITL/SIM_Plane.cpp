...
if(tnow > 130000  && n == 1)
    ...
    thrust *= thrust_scale;
//printf("1111111111111111111111111111  thrust= %f thrust_scale %f throttle %f\n", thrust, thrust_scale, throttle);
    ...
    static uint32_t count_aileron = 0;
	static uint32_t count_elevator= 0;
	static uint32_t count_rudder = 0;
	static uint32_t count_throttle = 0;
	static float   sum_aileron = 0;
	static float   sum_elevator = 0;
	static float   sum_rudder = 0;
	static float   sum_throttle = 0;
	
	static uint32_t i = 0;
	static uint32_t flag = 0;
	static uint32_t freq = 0;
	const uint32_t num = 10000;
	static float aileron_collect[num];
	static float elevator_collect[num];
	static float rudder_collect[num];
	static float throttle_collect[num];
	static float thrust_collect[num];
	float sum = 0;
	uint32_t k = 0;
	if(flag == 0)//  && tnow > 99460)
	{
		freq++;
	    if( freq == 25)
	    {
			...
			if(i<num)
		    {
		    	aileron_collect[i] = aileron;
				elevator_collect[i] = elevator;
				rudder_collect[i] = rudder;
				throttle_collect[i] = throttle;
				thrust_collect[i] = thrust;
			    i++;
				//if (aileron > 0.00000001)
				{
					count_aileron++;
					sum_aileron += fabs(aileron);
				}
				//if (elevator > 0.00000001)
				{
					count_elevator++;
					sum_elevator += fabs(elevator);
				}
				//if (rudder > 0.00000001)
				{
					count_rudder++;
					sum_rudder += fabs(rudder);
				}
				//if (throttle > 0.00000001)
				{
					count_throttle++;
					sum_throttle += fabs(throttle);
				}
		    }
			else
			{
				printf("the number of aileron etc. is out of %d\n", num);
			}
	    }
	}
	if(tnow > 158000)
	{
		...
		{
			flag++;
			printf("aileron etc. = [...] and total  number is %d\n",i);
			for(k=0;k<i;k++)
			{
				printf("%f ",aileron_collect[k]);
			}
			printf("\n");
			printf("elevator etc. = [...] and total  number is %d\n",i);
			for(k=0;k<i;k++)
			{
				printf("%f ",elevator_collect[k]);
			}
			printf("\n");
			printf("rudder etc. = [...] and total  number is %d\n",i);
			for(k=0;k<i;k++)
			{
				printf("%f ",rudder_collect[k]);
			}
			printf("\n");
			printf("throttle etc. = [...] and total  number is %d\n",i);
			for(k=0;k<i;k++)
			{
				printf("%f ",throttle_collect[k]);
			}
			printf("\n");
			printf("thrust etc. = [...] and total  number is %d\n",i);
			for(k=0;k<i;k++)
			{
				printf("%f ",thrust_collect[k]);
			}
			printf("\n");
			
			sum_aileron = sum_aileron / count_aileron;
			sum_elevator= sum_elevator/ count_elevator;
			sum_rudder = sum_rudder / count_rudder;
			sum_throttle = sum_throttle / count_throttle;
			sum = (sum_aileron + sum_elevator + sum_rudder + sum_throttle)/0.507767;  //  /0.412;
		    printf("Input energy: sum_aileron %f, sum_elevator %f, sum_rudder %f, sum_throttle %f, sum %f\n", (double)sum_aileron, (double)sum_elevator, (double)sum_rudder, (double)sum_throttle, (double)sum);
			//write into files
			AP::vehicle()->update_pso_cost(sum, 6);
		}
	}
